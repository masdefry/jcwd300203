...
const PropertyMediaUploader = () => {
  const [propertySelectedImgs, setPropertySelectedImgs] = useState<File[]>([]);

  // multiple image select
  const multipleImage = (e: any) => {
    // checking is same file matched with old stored array
    const isExist = propertySelectedImgs?.some((file1: any) =>
      selectedFiles(e)?.some((file2: any) => file1.name === file2.name)
    );
  ...
  const deleteImage = (name: any) => {
    const deleted = propertySelectedImgs?.filter((file: any) => file.name !== name);
